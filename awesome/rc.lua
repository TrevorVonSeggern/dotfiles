...
local terminals = { 'alacritty', 'gnome-terminal', 'xterm'}
local terminal = terminals[1]
-- smart set the terminal
for i,t in pairs(terminals) do
	if gears.filesystem.file_readable('/usr/bin/' .. t) then
		terminal = t
		break
	end
end
...
local cpu = lain.widget.cpu({
    settings = function()
        widget:set_markup(markup.fontfg(beautiful.font, "#e33a6e", string.format('%3d', cpu_now.usage) .. "% "))
    end
})
-- Coretemp
...
local temp = lain.widget.temp({
    settings = function()
        widget:set_markup(markup.fontfg(theme.font, "#f1af5f", string.format('%3.1f', coretemp_now) .. "°C "))
    end
})
-- MEM
...
local netupinfo = lain.widget.net({
    settings = function()
        ...
        widget:set_markup(markup.fontfg(theme.font, "#e54c62", string.format('% 8.1f', net_now.sent)))
        netdowninfo:set_markup(markup.fontfg(theme.font, "#87af5f", string.format('% 8.1f', net_now.received)))
    end
})
...
local function set_wallpaper(s)
    if beautiful.wallpaper then
        ...
        if type(wallpaper) == "function" then -- If wallpaper is a function, call it with the screen
